...
#### Setup `Helm` and `Kafka` in `guest VM`

*Helm* helps you manage Kubernetes applications — *Helm* Charts help you define, install, and upgrade even the most complex Kubernetes application. Install *Helm* in the guest VM using below command.
...
Now, you can add the chart repository below.
...
Now, Install the kafka helm chart
...
Now, once the Kafka pods are created and running, you can create topics.

``` bash
# Get the password for the kafka client
kubectl get secret udaconnect-kafka-user-passwords --namespace default -o jsonpath='{.data.client-passwords}' | base64 -d | cut -d , -f 1
```

Copy the password of kafka client then paste into `deployment/kafka-configmap.yaml`

### Steps

1. `kubectl apply -f deployment/db-configmap.yaml` - Set up environment variables for the pods
2. `kubectl apply -f deployment/db-secret.yaml` - Set up secrets for the pods
3. `kubectl apply -f deployment/kafka-configmap.yaml` - Set up environment variables for the kafka pods
4. `kubectl apply -f deployment/postgres.yaml` - Set up a Postgres database running PostGIS
5. `kubectl apply -f deployment/udaconnect-app.yaml` - Set up the service and deployment for the web app
6. `kubectl apply -f deployment/udaconnect-persons-service.yaml`  - Set up the service and deployment for the Persons Service
7. `kubectl apply -f deployment/udaconnect-connections-service.yaml`  - Set up the service and deployment for the Connections Service
8. `kubectl apply -f deployment/udaconnect-location-service.yaml`  - Set up the location service
9. `kubectl apply -f deployment/udaconnect-location-ingestion-service.yaml`  - Set up the location ingestion service
...
``` bash
    python fake_grpc_client.py    
```

 You can observe the progress of location ingestion by observing the logs of the  `location-service`  and  `location-ingestion`  microservice using the commands belo:

  ``` bash
    kubectl logs -f <location-service-pod-name>   
    kubectl logs -f <location-ingestion-pod-name>
  ```
...
Upon successfully launching the project, you are expected to observe nine deployments and an equal number of services within the Kubernetes environment. The following commands can be executed to verify the active pods and services:

* To display the list of pods, use:

  ```shell
  kubectl get pods
  ```

  This command should reveal a list of pods similar to the one depicted in the provided screenshot.

* To view the services, execute:

  ```shell
  kubectl get services
  ```

  This should return a list of services, akin to the example shown in the accompanying screenshot.

Furthermore, the project's components should be accessible through web browsers at the specified URLs:

* For accessing the OpenAPI Documentation, navigate to `http://localhost:30001/`.
* The API's base path is available at `http://localhost:30001/api/`.
* The Frontend ReactJS Application can be found at `http://localhost:30000/`.
* OpenAPI Documentation specific to the Persons API is located at `http://localhost:30002/`, with its base path at `http://localhost:30002/api/`.
* Lastly, the Connections API's OpenAPI Documentation and base path are accessible at `http://localhost:30003/` and `http://localhost:30003/api/`, respectively.

These steps and URLs ensure direct access to the project's various components, facilitating interaction with the APIs and the frontend application.
